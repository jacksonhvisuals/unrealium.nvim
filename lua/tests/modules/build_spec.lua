...
describe("modules.build", function()
	...
	describe("platform.run_command", function()
		it("returns Development run command as argv array", function()
			local cfg = tUtil.mock_config()
			local result = platform.run_command(cfg, "Development")
			assert.truthy(result)
			assert.truthy(result.cmd)
			assert.matches("UnrealEditor", result.cmd[1])
			assert.matches("MyProject.uproject", result.cmd[2])
		end)

		it("returns Debug run command with suffix", function()
			local cfg = tUtil.mock_config()
			local result = platform.run_command(cfg, "Debug")
			assert.matches("Linux%-Debug", result.cmd[1])
		end)

		it("includes extra_args in cmd array", function()
			local cfg = tUtil.mock_config()
			local result = platform.run_command(cfg, "Development", { "-norelativemousemode", "-log" })
			assert.equals("-norelativemousemode", result.cmd[3])
			assert.equals("-log", result.cmd[4])
		end)
	end)
